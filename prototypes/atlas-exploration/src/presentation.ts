...
export function kindLabel(node: AtlasNode) {
  ...
  return ({ domain: 'Domaine', capability: 'Capacité', reference: 'Référence', group: 'Groupe de présentation', object: 'Objet métier', document: 'Document', event: 'Événement' } as Record<string,string>)[node.kind] || node.kind;
}
