...
export function adaptPublication(input: RawPublication): PublishedModel {
  ...
  const nodes: AtlasNode[] = raw.nodes.map(node => {
    ...
    return freezeDeep({
      id: node.id, name: textField(fields.name) || node.id, kind: node.kind,
...
export function searchModel(model: PublishedModel, query: string): AtlasNode[] {
  ...
  return model.nodes.filter(node => {
    const haystack = normalize(`${node.id} ${node.kind} ${fieldText(node.fields)} ${lineageOf(model, node.id).map(parent => parent.name).join(' ')}`);
