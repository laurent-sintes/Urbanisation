...
Source : `modeles/backlog/model.yaml` ; SHA-256 `bc6331a9e199a4723199f39b26ceb3e94ab5caded35e4075fd91d8054acd7347`.
...
Sources : U237, U238, U240, U242, U243, U244, U247, U435
...
Sources : U245, U246, U247, U290, U435
...
Sens : Le consommateur tient compte des quantités réservées et des conditions de cet engagement pour ses possibilités ou décisions ; Reservation porte le blocage des usages concurrents.

Conditions : Lorsque les ressources ou commandes du périmètre sont concernées ; U436 distingue affectation et réservation. | Préserver la référence des quantités afin de ne pas les décompter plusieurs fois ; les règles de consommation et de libération restent à préciser.

Effets : La réservation porte le blocage concurrent ; aucune réservation automatique n’est créée par la simple affectation. | Le consommateur conserve sa responsabilité et respecte les protections et restrictions applicables.

Sources : U286, U290, U436
...
Sens : Le consommateur utilise les liens entre ressources et commandes pour connaître leur affectation ; ces liens seuls ne bloquent pas les usages concurrents.
...
Sources : U377, U378, ELM228, CMP139, U435
...
Sources : U382, U384, U385, ELM232, CMP143, U435
...
Sources : U390, U391, ELM235, CMP146, U435
...
Sources : U424, ELM251, CMP162, U435

## REL-AUDIT-U435-PLNK-01

`D03.p` Order Backlog Planning → `D03.o` Fulfillment Plan Decision

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Order Backlog Planning mobilise Fulfillment Plan Decision pour obtenir des scénarios collectifs cohérents, comparer leurs hypothèses et préparer le scénario retenu.

Conditions : Lorsqu’un travail collectif du carnet nécessite de construire ou de réexaminer un scénario.

Effets : Scénarios et impacts explicables | Aucun engagement ou changement d’Order par la seule simulation.

Sources : U398, U399, U413, U417, U435

## REL-AUDIT-U435-PLNK-02

`D06.d` Process Orchestration → `D04.o` Order Lifecycle Management

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Process Orchestration consomme les autorisations de prise en charge et restrictions de progression gouvernées par Lifecycle pour coordonner les prestations admissibles.

Conditions : Quantités ou lignes explicitement autorisées | Hold ou retrait d’autorisation traité selon sa portée | Réalisations déjà acquises conservées.

Effets : Prise en compte explicable de l’autorisation et de ses changements | Aucun démarrage physique présumé | Aucun rollback physique universel.

Sources : U398, U399, U410, U420, U435

## REL-AUDIT-U435-PLNK-03

`D04.r` Consignment Replenishment Order → `D07.c` Service Reconciliation

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Consignment Replenishment Order utilise les résultats imputés aux prestations pour expliquer les quantités reçues et le reste à recevoir sans transformer la réception en acquisition.

Conditions : Réception reconnue et rattachée aux quantités de la demande | Écarts et résultats partiels conservés.

Effets : Reste à recevoir explicable | Propriété du fournisseur préservée selon l’accord | Aucun stock créé par le seul état de la demande.

Sources : U398, U399, U395, U409, U435

## REL-AUDIT-U435-PLNK-04

`D01.h` Consigned Inventory Management → `D01.g` Record Inventory Movements

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Consigned Inventory Management mobilise les faits et écritures de stock qualifiés pour rattacher réception, consommation et changement de propriété aux quantités concernées ; Record Inventory Movements garde l’enregistrement des mouvements.

Conditions : Événement reconnu concernant un stock sous accord | Propriétaire et détenteur identifiés | Changement de propriété seulement selon l’accord.

Effets : Propriété et mouvement physique distingués | Traçabilité du fait justifiant le changement | Aucune consommation ou acquisition déduite automatiquement d’une simple demande.

Sources : U398, U399, U395, U401, U435

## REL-AUDIT-U435-PLNK-05

`D03.n` Promise Management → `BHV078` Supplier Confirmation

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Promise Management consomme les réponses et engagements fournisseur qualifiés par Supplier Confirmation pour apprécier les conséquences sur les promesses concernées et traiter leur éventuelle révision.

Conditions : Un achat contribue à la satisfaction concernée. | Distinguer demande, proposition fournisseur et engagement accepté. | Relier les quantités et échéances concernées.

Effets : Promesse réexaminable avec provenance du changement | Aucune modification automatique de la promesse à l’acceptation fournisseur | Refuser un report ne recrée pas une capacité fournisseur.

Sources : U398, U399, U403, U435

## REL-AUDIT-U435-PLNK-06

`D05.f` Inventory Planning → `D04.r` Consignment Replenishment Order

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Inventory Planning mobilise Consignment Replenishment Order pour formuler ou ajuster une demande d’apport consigné autorisée et connaître le résultat de sa prise en compte.

Conditions : Scénario retenu comprenant un apport sous accord de consignation | Besoin d’implantation ou de réassort identifié | Mutations d’Order autorisées.

Effets : Demande créée ou modifiée avec traçabilité au choix retenu | Refus et prise en compte partielle visibles | Aucun engagement d’achat ni arrivée physique implicite.

Sources : U398, U399, U395, U427, U435

## REL-AUDIT-U435-CONSIGNED-LIFECYCLE

`D04.r` Consignment Replenishment Order → `D04.o` Order Lifecycle Management

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Consignment Replenishment Order mobilise Order Lifecycle Management pour gouverner les évolutions autorisées de sa demande, en conservant lignes, quantités et engagements distincts.

Conditions : Lorsqu’une mutation de la demande est nécessaire et autorisée selon l’accord applicable.

Effets : Conserver une demande cohérente sans créer un engagement d’achat ni réaliser la prestation.

Sources : U395, U398, U435

## REL-AUDIT-U435-CONSIGNED-STRUCTURING

`D04.r` Consignment Replenishment Order → `D04.n` Order Structuring

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Consignment Replenishment Order mobilise Order Structuring pour les découpages et compositions autorisés de la demande, en conservant sa filiation et les quantités concernées.

Conditions : Si la demande doit être scindée ou intégrée à une composition persistante.

Effets : Rendre la structure explicable sans compter deux fois le même apport ni imposer un Order chapeau.

Sources : U395, U398, U435

## REL-AUDIT-U435-ARCHIVING-CONSIGNED

`D04.q` Order Archiving → `D04.r` Consignment Replenishment Order

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Order Archiving utilise le contenu, les versions et les traces de Consignment Replenishment Order pour assurer leur conservation selon la politique applicable.

Conditions : Lorsque les demandes ou leurs versions doivent être conservées selon les règles applicables.

Effets : Préserver les traces sans reprendre la gestion courante de la demande ni confondre clôture et archivage.

Sources : U395, U398, U435

## REL-AUDIT-U435-CONSIGNED-TRACKING

`D01.h` Consigned Inventory Management → `D01.f` Inventory Tracking

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Consigned Inventory Management utilise les états de stock connus pour appliquer les conditions de l’accord sur le propriétaire, la détention, les droits d’usage et les échéances.

Conditions : Stock identifié comme consigné et conditions de l’accord disponibles ; état connu distingué des ressources futures.

Effets : Appliquer le régime au bon périmètre sans inventer une réception ou modifier le maître de l’accord.

Sources : U395, U401, U435

## REL-AUDIT-U435-CONSIGNED-PURCHASE

`D01.h` Consigned Inventory Management → `D04.j` Purchase Order

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Consigned Inventory Management mobilise Purchase Order lorsqu’une acquisition autorisée du stock consigné nécessite un acte d’achat, et utilise son résultat pour suivre l’issue de l’accord.

Conditions : Uniquement si les conditions de l’accord et la suite retenue prévoient un achat ; aucune acquisition automatique à la réception.

Effets : Rattacher l’achat éventuel au stock concerné sans assimiler détention et propriété ni absorber la finance.

Sources : U395, U401, U435

## REL-AUDIT-U435-CONSIGNED-SUPPLIER-RETURN

`D01.h` Consigned Inventory Management → `D04.m` Supplier Return

Type : `relates-to` ; rôle : `needs` ; statut : `proposed`.

Sens : Consigned Inventory Management mobilise Supplier Return lorsqu’une issue autorisée de la consignation prévoit le renvoi des marchandises au fournisseur.

Conditions : Lorsque la suite retenue et l’accord permettent une reprise physique fournisseur.

Effets : Suivre la reprise via l’Order responsable sans déduire sa réalisation physique du seul choix de sortie.
