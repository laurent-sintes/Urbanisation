...
export function ReferenceLink({ kind = 'model', target, anchor, children, className }: { kind?: Kind; target: string; anchor?: string; children: ReactNode; className?: string }) {
  ...
  const description = plainInlineText(publicText(term?.short_description || String(node?.fields.short_description || node?.purpose || node?.definition || 'Description non renseignée.')));
