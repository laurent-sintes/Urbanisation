...
export function BusinessSheet({ model, node }: { model: PublishedModel; node: AtlasNode }) {
  ...
  const sections = [fields.scope && ['scope', 'Périmètre'], relations.length > 0 && ['interactions', 'Interactions'], behaviors.length > 0 && ['behaviors', 'Comportements']].filter(Boolean) as string[][];
