...
- **Maille des comportements (U282)** : rechercher les mécanismes métier et leurs effets, plutôt qu’une liste d’opérations sur un concept. Pour Supply Protection, le découpage allocation/réallocation/libération et la liste de 17 opérations sont en réexamen ; les détails fonctionnels alimentent la conception produit. Ne pas réappliquer ce découpage historique ni refondre automatiquement ATP ou Planning.
...
- **Planning mobilise les décisions**, qui restent des capacités distinctes, pas ses comportements enfants. U269/U271 adoptent pour Inventory Planning six comportements : **Scenario Construction, Scenario Simulation, Scenario Impact Analysis, Scenario Evaluation, Scenario Validation et Scenario Application**. Simulation produit la situation opérationnelle projetée ; Impact Analysis quantifie et explique les conséquences en indicateurs ; Evaluation apprécie le compromis. Les indicateurs peuvent déjà provenir de la simulation, sans recalcul imposé. Cela précise les trois verbes U229 ; la généralisation à toute capacité Planning reste proposée dans MOD002.
...
Un transfert peut servir la satisfaction d’un Order ou l’optimisation du stock : l’opération commune ne fusionne pas D03 et D05. U269 attribue explicitement à Scenario Application sous Inventory Planning le déclenchement des actions retenues et la connaissance de leur prise en compte **via les capacités opérationnelles responsables** : Supply Protection pour les protections, D04 pour les Orders et D06 pour les prestations. Leurs responsabilités restent distinctes ; validation du scénario, application transactionnelle et réalisation physique ne se confondent pas. Le calcul des besoins nets appartient à Replenishment Decision (D05.b retirée).
...
- Inventory Planning : [six comportements adoptés U269/U271](connaissance/35-comportements-inventory-planning.md) ; portées et empreintes dans `modeles/backlog/d05-refactoring.yaml`, sections `scenario_behaviors_U269` et `scenario_impact_U271`. Synthèse de capacité, exemples et périmètres ajoutés restent proposés.
...
- Plan d’amélioration et arbitrages : [audit U265/U266](audits/2026-09-17-audit-comportements/rapport.md), [propositions structurées](modeles/backlog/behavior-audit.yaml).
