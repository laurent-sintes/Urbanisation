...
export function BusinessSheet({ model, node }: { model: PublishedModel; node: AtlasNode }) {
  ...
  const sections = [fields.scope && ['scope', 'Périmètre'], relations.length > 0 && ['interactions', 'Interactions'], behaviors.length > 0 && ['behaviors', 'Comportements'], ['market_comparisons', 'Marché']].filter(Boolean) as string[][];
  ...
  return <article key={node.id} className="business-sheet sheet" data-testid="business-sheet" aria-label={`Fiche métier de ${node.name}`}>
    ...
    <MarketComparisons id={`field-${node.id}-market_comparisons`} entries={node.fields.market_comparisons as readonly MarketComparison[] | undefined}/>
