# Digital Service Visibility — proposition U306
...
**Besoin exprimé.** "Le mandat de traçabilité numérique reste également acquis." => Je pense que c'est une visibilité explicite. Si on appelle par exemple un service numérique de vérification quelconque comme un code barre, un dispositif anti fraude, une vérification d'identité, une production documentaire numérique etc. on doit pouvoir en avoir une visibilité claire.

**Définition proposée.** Rendre visibles la progression et les résultats des prestations numériques sollicitées, en reliant chaque demande, ses tentatives et ses réponses aux objets et opérations métier concernés.

**Périmètre.** Vérification de code-barres, contrôle antifraude, vérification d’identité, production documentaire et autres prestations numériques. Exposer sollicitation, prise en charge connue, traitement en cours, résultat ou absence de réponse ; ces situations ne constituent pas un cycle universel imposé.

**Bénéfice de décomposition.** Une demande acceptée peut rester sans résultat ; un appel techniquement réussi peut produire un résultat métier défavorable. Relier demande, réponses asynchrones, tentatives et objet métier permet de savoir si la prestation attendue a réellement abouti et avec quel effet.

**Exemple.** Le service antifraude a répondu correctement, mais son résultat est une suspicion à examiner. La prestation est terminée ; la commande peut rester en attente. Un service documentaire peut avoir accepté la demande sans avoir encore produit le document.

**Frontières.** La visibilité constate faits, progression et résultats ; elle ne décide ni l’acceptation d’un risque ni la suite de l’Order. Orchestration et adaptation gardent leurs responsabilités. D14 décrit les services offerts ; ce comportement observe leurs sollicitations effectives. Ne pas créer un comportement par service ou interface. Les traces numériques des opérations physiques peuvent alimenter leur visibilité sans changer leur nature.

**Marché.** Microsoft documente le suivi des exécutions et la corrélation avec des étapes métier ; Camunda emploie Process Observability. Ces périmètres étayent le besoin, mais les sources consultées ne donnent pas un intitulé normalisé exactement équivalent. Digital Service Visibility est un libellé FLOW proposé, plus ciblé que le suivi de tout un processus et cohérent avec les trois Visibility adoptés.

[S48 — Microsoft](https://learn.microsoft.com/en-us/azure/logic-apps/view-workflow-status-run-history), [S49 — Microsoft](https://learn.microsoft.com/en-us/azure/business-process-tracking/overview), [S50 — Camunda](https://camunda.com/platform/observability/)

**Portée.** Besoin explicite issu de Laurent ; nom, définition éditoriale, bénéfice formulé et quatrième rattachement proposés. Accord U305 sur les trois comportements logistiques inchangé.

Catalogue et publications inchangés.
