...
export function Sidebar({ model, route, open, mobile, searchRef, onClose, onNavigate, onSearch, onOpenGlossary, onOpenPrinciples }: Props) {
  ...
  const matches = searchModel(model, route.query).filter(n => (!route.type || n.kind === route.type));
  ...
  return <aside ref={panel} id="atlas-tree-panel" className={`sidebar ${open ? 'open' : ''}`} aria-label="Navigation du modèle" aria-modal={mobile && open ? true : undefined} role={mobile && open ? 'dialog' : undefined}>
    ...
    <button className="glossary-nav" onClick={() => onOpenGlossary('model')} aria-current={route.view === 'glossary' && route.glossary !== 'meta' ? 'page' : undefined}><BookOpen size={19}/>Glossaire métier</button>
    <button className="glossary-nav" onClick={() => onOpenGlossary('meta')} aria-current={route.view === 'glossary' && route.glossary === 'meta' ? 'page' : undefined}><BookOpen size={19}/>Glossaire du méta modèle</button>
    <button className="glossary-nav principles-nav" onClick={onOpenPrinciples} aria-current={route.view === 'principles' ? 'page' : undefined}><Lightbulb size={19}/>Comprendre le méta modèle</button>
    ...
    <div className="search-filters">
      <select id="fa-type" aria-label="Type d’élément" value={route.type} onChange={e => onSearch({ type: e.target.value })}><option value="">Tous les types</option>{kinds.map(kind => <option key={kind} value={kind}>{kind === 'capability' ? 'Capacités' : kindLabel(model.nodes.find(n => n.kind === kind)!) === 'Univers' ? 'Groupes et niveaux' : kindLabel(model.nodes.find(n => n.kind === kind)!)}</option>)}</select>
    </div>
    ...
    <div className="sidebar-bottom"><span className="live-dot" /><span>{model.nodes.length} éléments · {model.nodes.filter(n => n.kind === 'capability').length} capacités{model.nodes.some(n => n.kind === 'behavior') && <> · {model.nodes.filter(n => n.kind === 'behavior').length} comportements</>}</span></div>
  </aside>;
}
