...
Accords : enregistrer les validations explicites en amont avec `scripts/record_decision.py` et des champs nommés ; ne jamais les déduire de lifecycle. Le registre YAML conserve valeurs et contexte, sans version de release imposée. Les reprises automatiques exigent valeurs et contexte métier inchangés ; seules les métadonnées et références éditoriales identifiées peuvent varier. Les champs inconnus restent significatifs.
