...
La hiérarchie courante est **Domain → Purpose → Capability → Behavior** (U624). Domain désigne le grand périmètre métier, comme Supply Chain Orchestration. Purpose (Finalité) désigne une finalité métier durable au sein d’un Domain, autour de laquelle sont regroupées les responsabilités et capacités qui concourent à sa réalisation. Purpose remplace le nom Area U482 ; conserver les identifiants et le vocabulaire des publications historiques. Les codes techniques `area` restent compatibles avec le schéma existant jusqu’à la mise en cohérence technique. Une capacité décrit ce que sait faire durablement l’entreprise, indépendamment de son organisation et de ses outils. Actions à maille large, décisions éventuellement plus fines. Une fonction de produit ne suffit pas à définir une capacité.
...
Construire l’arbre par relations explicites. `contains`, `presents` et liens métier diffèrent. U626 applique au backlog six Purposes ; [le lot de consolidation](modeles/backlog/model-consolidation-U626.yaml) distingue les décisions acquises et les formulations/rattachements proposés. Reference & Policy Management conserve `business-references`, présente les sept référentiels distincts U509 et contient les capacités de policies. Chaque référentiel conserve ingestion et visibilité ; les références organisent les sujets sans nouveau niveau descriptif ni fusion de données. Les Purposes retirés D05/D15/D17 restent tracés, leurs capacités conservant leurs identifiants. Ne jamais réutiliser un identifiant retiré. Qualifier les liens transversaux avec une expression métier, leurs conditions, effets et sources disponibles. Une dépendance n’est pas une décomposition.
...
Les captures [du 19 septembre](audits/2026-09-19-performance/AGENTS-before-optimization.md) et [du 22 septembre](audits/2026-09-22-instructions/AGENTS-before-optimization.md) préservent les instructions remplacées comme preuves, pas comme règles actives.
...
Release U504 : le parcours `scripts/release.py --source SOURCE --activate` construit une fois le candidat, prépare, publie et vérifie Atlas ; durées et compte rendu sont générés. Aucun rapport préalable systématique. Si un examen métier est nécessaire, utiliser le dossier fourni puis une seule préparation finale. Lire `report.json` et le candidat figé sans reconstruire pour relire. Contrôles de publication obligatoires.

Accords : utiliser `scripts/record_decision.py` sur le lot final, avant publication ; pour plusieurs accords, utiliser `record_intents` en une écriture. Répéter `--fields` pour plusieurs champs. Pour un nœud, les champs sélectionnables sont ceux de `fields` ; `kind` appartient au contexte. Le registre conserve valeurs et contexte sans version de release imposée. Une reprise automatique exige valeurs et contexte métier inchangés ; seules les métadonnées et références éditoriales identifiées peuvent varier. Les champs inconnus restent significatifs.

Le registre est découpé : `decision-intents.yaml` est son index, `decision-intents/active/` et `archive/` contiennent les captures immuables. Lire le registre complet avec `scripts.decision_registry.read_registry`, jamais interpréter l’index comme une liste d’accords complète. Ne pas modifier les fragments à la main. L’archivage ne vaut pas adoption ; les contrôles exhaustifs et la capture des fragments restent obligatoires à la publication.

`validate_models.py` contrôle aussi le registre complet, ses sources, empreintes et résumés, ainsi que la conservation des accords et suspensions du snapshot courant. Le compteur `decision_unreferenced_shards` signale les fragments non référencés sans les adopter ni les supprimer. La reprise d’un accord sur un contexte métier modifié reste un contrôle de préparation de release, pas une condition de validité du backlog de travail.
...
## Publication sobre — consigne du 23 septembre 2026

Conserver la base de connaissance marché, le modèle et le glossaire publiés, les sources et la portée des accords. Ne plus copier les dossiers de réflexion, variantes abandonnées et annexes de travail dans chaque release. Le staging est temporaire ; la release contient les artefacts utiles à sa lecture et à sa vérification. Les nouvelles captures d’accords gardent les valeurs approuvées et des empreintes de contexte, sans recopier le glossaire ou les fiches voisines. Les anciens formats restent lisibles. Les rapports comparent les éléments par identifiant et référencent les preuves sans les dupliquer. Diagnostiquer toutes les intentions périmées en une passe.

Laurent a explicitement autorisé l’allègement des rapports et annexes de v020, puis la réécriture de son commit local refusé par GitHub ; modèle et accords restent identiques. Cette exception ne donne pas d’autorisation générale de réécrire les autres publications. Git LFS ne remplace pas la réduction des artefacts superflus.
