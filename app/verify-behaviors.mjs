...
try {
  ...
  const catalog = await (await page.request.get(base + '/api/releases')).json();
  await page.route('**/api/releases', route => route.fulfill({ json: { ...catalog,
    versions: [{ version: fixture.version, revision: 0 }, ...catalog.versions] } }));
  await page.route('**/api/model*', route => new URL(route.request().url()).searchParams.get('version') === fixture.version
  ...
  assert.equal((await (await page.request.get(base + '/api/model')).json()).version, published.version);
