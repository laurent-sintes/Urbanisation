...
export function DependenciesPane({ model, focusId, relationId, settings, onSettings, onSelectRelation, onFocus, onRead, onOpenSource }: Props) {
  ...
  return <section ref={pane} className="map-panel dependencies-pane" data-testid="dependencies-pane" aria-label="Graphe des relations métier">
    ...
    <div className="dependency-inspector" data-testid="dependency-inspector">
      ...
      {relation && <><RelationDetails model={model} relation={relation} onOpenSource={onOpenSource}/>{!projection.relations.some(r => r.id === relation.id) && <p className="dependency-help">Cette relation appartient à la publication, mais reste hors du périmètre affiché.</p>}</>}
    </div>
  </section>;
}
