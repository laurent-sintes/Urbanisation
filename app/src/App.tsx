...
export function App() {
  ...
  const openGlossary = useCallback(() => {
    changeRoute({ view: 'glossary', node: '', term: '', principle: '', section: '', scope: '', relation: '', source: '', anchor: '', sourceId: '' });
    setDrawer(false);
  ...
  return <ModelLinksProvider value={{ model: model || null, route, onFollow: followReference }}><div className="atlas-shell" style={{ '--sidebar': `${width}px` } as CSSProperties}>
    ...
    {model && <Sidebar model={model} route={route} open={drawer} mobile={mobile} searchRef={search} onClose={closeDrawer} onNavigate={navigate} onOpenGlossary={openGlossary} onOpenPrinciples={openPrinciples} onSearch={changes => changeRoute(changes, true)} />}
    ...
    <main id="fa-main" className={`workspace ${!model ? 'without-model' : ''}`} inert={mobile && drawer} aria-busy={loading}>
      ...
      {!model ? <div className="empty-state"><Compass size={34} /><h1>{loading ? 'Ouverture du modèle…' : 'Publication indisponible'}</h1><p>{loading ? 'Chargement de l’Urbanisation publiée.' : 'Réessaie de charger la publication.'}</p>{!loading && <button className="secondary-button" onClick={reload}>Réessayer</button>}</div> : <>
        ...
        <header className="page-heading"><div className="heading-icon">{headingNode ? <NodeIcon node={headingNode} size={30} framed /> : <span className="node-icon framed tone-universe">{view === 'principles' ? <Lightbulb size={30} /> : <Compass size={30} />}</span>}</div><div>
          <div className="eyebrow"><span>{headingNode ? kindLabel(headingNode) : view === 'principles' ? 'COMPRENDRE LA MODÉLISATION' : view === 'glossary' ? 'LE VOCABULAIRE PUBLIÉ' : 'LE MODÈLE PUBLIÉ'}</span>{headingNode && <span>{headingNode.id}</span>}{headingNode && <span className={`pill ${headingNode.status}`}>{statusLabel(headingNode)}</span>}</div>
          <h1 id="page-title" ref={heading} tabIndex={-1}>{view === 'principles' ? 'Les clés du modèle' : view === 'glossary' ? 'Glossaire' : headingNode?.name || 'Urbanisation'}</h1>
        ...
        <div id="atlas-view" role={referenceView ? 'region' : 'tabpanel'} aria-labelledby={referenceView ? 'page-title' : `tab-${view === 'sheet' && !selected ? 'map' : view}`}>
          {view === 'principles' ? <Suspense fallback={<p role="status">Ouverture des clés du modèle…</p>}><ModelingGuidePage key={model.version} model={model} selected={route.principle} onSelect={principle => changeRoute({ principle })} /></Suspense> : view === 'glossary' ? <GlossaryPage model={model} selected={route.term}/> : view === 'sheet' && selected ? <BusinessSheet model={model} node={selected} onOpenSource={openSource} /> : view === 'relations' ? <Suspense fallback={<div className="graph-canvas empty-state">Ouverture des relations…</div>}><DependenciesPane key={`${model.version}:${route.node}`} model={model} focusId={selected?.id} relationId={route.relation} settings={route} onSettings={changes => changeRoute(changes)} onSelectRelation={relation => changeRoute({ relation }, true)} onFocus={node => changeRoute({ node, scope: '', relation: '', view: 'relations', graphDepth: node ? 1 : 0 })} onRead={read} onOpenSource={openSource}/></Suspense> : <>
            <section className="map-panel" ref={mapPanel} aria-label="Carte du modèle">
              ...
              <Suspense fallback={<div className="graph-canvas empty-state">Ouverture de la carte…</div>}><ReactFlowPane model={model} selectedId={selected?.id || ''} scopeId={scopeId} onSelect={select} onExplore={explore} onRead={read} perspective={perspective} /></Suspense>
