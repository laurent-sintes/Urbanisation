...
import { ArrowUpRight, FileText, Network, LayoutGrid } from 'lucide-react';
...
function Canvas(props: ReactFlowPaneProps) {
  ...
  useEffect(() => {
    ...
    const inputs = items.map(n => ({ id: n.id, width: 300, height: 220 }));
    ...
    const placement: Promise<ElkNode> = graph.mode === 'hierarchy' || !edges.length ? Promise.resolve({
      id: 'canvas', width: columns * 300 + (columns - 1) * 28 + 32,
      height: rows * 220 + Math.max(0, rows - 1) * 28 + 32,
      children: inputs.map((item, index) => ({ ...item, x: 16 + (index % columns) * 328, y: 16 + Math.floor(index / columns) * 248 }))
    }) : import('elkjs/lib/elk.bundled.js').then(({default: ELK}) => new ELK().layout<ElkNode>({ id: 'canvas', layoutOptions: {
    ...
    placement.then(result => {
      ...
      for (const position of result.children || []) {
        ...
        nodes.push({ id: item.id, type: 'business', data: { item }, position: { x: (position.x || 0) + (group ? 14 : 0), y: (position.y || 0) + (group ? 54 : 0) }, ...(group ? { parentId: `group:${group.id}`, extent: 'parent' as const } : {}), width: 300, height: 220, style: { width: 300, height: 220 }, draggable: false, ariaLabel: `${kindLabel(item)} ${item.name}` });
      }
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => void fitView({ padding: .06, maxZoom: 1, duration: 0 }));
  ...
  const emptyScope = mode === 'map' && scopeId && model.nodeById.get(scopeId);
  ...
  return <div ref={container} className="graph-canvas" data-testid="react-flow-canvas" data-layout-ms={layout.ms}>
