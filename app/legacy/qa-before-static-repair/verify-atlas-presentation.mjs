...
try {
  ...
  const status = await (await page.request.get(base + '/__atlas__/identity.json')).json();
  assert.equal(status.appName, 'FLOW Atlas'); assert.equal(status.mode, 'static');
  const model = await (await page.request.get(await staticModelUrl(base))).json();
  const guide = await (await page.request.get(base + '/data/' + model.version + '/guide.json')).json();
  ...
  await page.route('**/data/*/model.json', route => route.fulfill({ json: fixture }));
