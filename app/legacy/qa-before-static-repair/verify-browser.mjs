...
try {
  ...
  observed.on('response', response => {
    if (response.status() >= 400 && !(intentionalFailure && response.status() === 503 && response.url().endsWith('/model.json'))) httpErrors.push({ url: response.url(), status: response.status() });
  });
  };
  monitorPage(page);
  const api = await (await context.request.get(await staticModelUrl(base))).json();
  const catalog = await (await context.request.get(base + '/data/index.json')).json();
  ...
  const historical = await (await context.request.get(await staticModelUrl(base, '2026-09-13.2'))).json();
  ...
  const catalogMatcher = url => url.pathname === '/data/index.json';
  const modelMatcher = url => url.pathname.endsWith('/model.json') && url.pathname.split('/').at(-2) === updated.version;
  ...
  await historicalPage.waitForResponse(async response => new URL(response.url()).pathname === '/data/index.json' && (await response.json()).current_version === updated.version, { timeout: 12000 });
