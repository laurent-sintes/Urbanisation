...
import { businessExamples } from './src/examples.ts';
...
const output = resolve(root, '.runtime/verify-market', new Date().toISOString().replace(/[:.]/g, '-'));
...
try {
  ...
  const marketTab=page.getByRole('tab',{name:'Sources d’inspiration',exact:true});
  ...
  await page.route('**/*',async route=>{
    ...
    if(url.pathname==='/data/index.json')return route.fulfill({json:{current_version:fixture.version,versions:[{version:fixture.version},{version:old.version}]}});
    if(url.pathname.endsWith('/model.json'))return route.fulfill({json:url.pathname.split('/').at(-2)===old.version?old:fixture});
    if(url.pathname.endsWith('/guide.json'))return route.fulfill({json:{schema_version:'1.0.0',publication_version:url.pathname.split('/').at(-2),status:'unavailable',message:'Fixture isolée.'}});
  ...
  const inspirationNode=fixture.nodes.find(node=>node.fields?.market_inspiration);
  if(inspirationNode){
    await preview(inspirationNode.id,'market');
    const inspiration=page.locator('.market-inspiration');await inspiration.waitFor();
    assert.equal(await inspiration.locator('.inspiration-table tbody tr').count(),inspirationNode.fields.market_comparisons.length+1);
    assert.ok(await inspiration.locator('.inspiration-choice').isVisible());
    assert.ok(await inspiration.locator('.inspiration-synthesis').isVisible());
    assert.equal(await inspiration.locator('.inspiration-example').count(),inspirationNode.fields.market_inspiration.examples.length);
    assert.ok(await inspiration.locator('.inspiration-example .market-source a').first().isVisible());
    assert.equal(await inspiration.locator('.inspiration-flow td').first().innerText(),inspirationNode.fields.name);
    assert.equal(await inspiration.locator('.inspiration-sources details[open]').count(),0);
    assert.doesNotMatch(await inspiration.innerText(),/\bU\d+\b|\bCMP\d+\b|\bELM\d+\b/);
    await page.setViewportSize({width:390,height:844});await noOverflow();
    const tableRegion=inspiration.getByRole('region',{name:'Comparer les sources d’inspiration et FLOW'});
    await tableRegion.focus();assert.ok(await tableRegion.evaluate(element=>element===document.activeElement));
    await page.keyboard.press('ArrowRight');
    await page.setViewportSize({width:1440,height:1000});
    await shot('fixture-inspiration');
    checks.push('Fixture structurée : choix, comparaison des concepts et FLOW, synthèse, exemple attribué et tableau accessible au clavier.');
  }
