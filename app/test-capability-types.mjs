...
test('explicit types drive labels and seven distinct icons; never names or IDs', () => {
  assert.equal(Object.keys(capabilityTypes).length, 7);
  assert.equal(new Set(Object.values(capabilityTypes).map(type => type.icon)).size, 7);
...
test('all seven types group stably; unknown types stay neutral and non-capabilities retain order', () => {
  ...
  assert.equal(output.filter((_, i) => startsCapabilityTypeSection(output, i)).length, 7);
